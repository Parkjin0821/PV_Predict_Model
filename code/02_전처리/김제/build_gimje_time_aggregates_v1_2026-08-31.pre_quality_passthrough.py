...
def apply_night_zero(aligned: pd.DataFrame) -> pd.DataFrame:
    ...
    # 야간0은 전력에만 적용한다. 온도·전압·역률 등의 결측은 그대로 둔다.
    x["quality_status_after_night"] = np.select(
        [x["was_observed"], x["was_interpolated"], x["night_zero_any"]],
        ["observed", "interpolated_le10min", "night_zero_physical"],
...
def build_plant_five(x: pd.DataFrame) -> pd.DataFrame:
    ...
    observed = x.pivot(index="grid_time_kst", columns="inverter_number", values="was_observed")
    interp = x.pivot(index="grid_time_kst", columns="inverter_number", values="was_interpolated")
    night = x.pivot(index="grid_time_kst", columns="inverter_number", values="night_zero_any")
    ...
    out["interpolated_inverter_count"] = interp.fillna(False).sum(axis=1).astype("int8")
    out["night_zero_inverter_count"] = night.fillna(False).sum(axis=1).astype("int8")
    ...
    out["quality_status"] = np.select(
        ...
        ["complete_with_night_zero", "complete_with_short_interpolation", "complete_observed"],
...
def main() -> None:
    ...
    required = {"grid_time_kst", "inverter_number", "ac_power_kw", "dc_power_kw", "was_observed", "was_interpolated"}
    ...
    expected = pivot.sum(axis=1, min_count=len(EXPECTED_INVERTERS))
