"""부안 격자예보(REH·POP·SKY) 백필을 기존 결합자료에 추가 결합.

★선행조건★: backfill_buan_grid_forecast_v1_2026-08-31.py 실행 완료로
`부안_격자예보_REH_POP_SKY_236일.csv`가 존재해야 한다. 없으면 즉시
FileNotFoundError로 멈춘다(임의로 건너뛰거나 결측 채움 없음).

## 발행시각 정합성(실측 확인, 08-31)
기존 NWP 결합(부안_과거발전_ASOS_NWP_결합.parquet)의 공식 발행시각은
`발행시각_kst`가 전부 **10:00 KST**로 실측 확인됨(236행 전부 동일).
반면 격자예보(동네예보) API는 항상 **05:00 KST**(tmfc=발표일+"05")에
발표된 값을 조회한다(grid_forecast_resume_20260606.py 고정값) - 05:00은
10:00보다 이르므로, 같은 발표일의 격자예보값을 NWP 발행시각(10:00)
시점의 특성으로 붙여도 미래누출이 아니다(05:00 시점에 이미 발표된
값이라 10:00에는 당연히 알려져 있음).

따라서 **정확한 발행시각(clock) 일치가 아니라 발표일(issue_date) +
대상시각(target_time_kst) 일치**로 결합한다 - 두 API의 발행시각
관례(05시 vs 10시)가 서로 다르기 때문에 발행시각 자체로 조인하면
안 맞는다. 이 근거는 backfill 실행 후 실제 CSV로 재검증할 것.
"""
...
GRID_CSV = Path(
    r"C:\Users\u-cube\JIN\코덱스\결과물\예측모델\부안\기상과거백필_v1_2026-08-28\GRID"
    r"\부안_격자예보_REH_POP_SKY_236일.csv"
)
OUT_DIR = Path(
    r"C:\Users\u-cube\JIN\코덱스\결과물\예측모델\부안\과거발전_기상결합_v1_2026-08-28"
)
OUT_PARQUET = OUT_DIR / "부안_과거발전_ASOS_NWP_GRID_결합_v1_2026-08-31.parquet"
...
def main() -> None:
    ...
    if not a.combined.is_file():
        raise FileNotFoundError(f"기존 결합자료 없음: {a.combined}")
    if not a.grid_csv.is_file():
        raise FileNotFoundError(
            f"격자예보 백필 CSV 없음 - backfill_buan_grid_forecast_v1_2026-08-31.py"
    ...
    # 매칭이 안 된 행이 있으면(발표일 범위 불일치 등) 조용히 넘어가지 않고
    # 그대로 드러낸다 - 임의보간 금지 원칙.
    ...
    (a.output_dir / "부안_GRID결합_요약.json").write_text(
