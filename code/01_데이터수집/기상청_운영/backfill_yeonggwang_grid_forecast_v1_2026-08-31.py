"""영광 김삿갓1 격자예보 REH·POP·SKY 710일 과거 백필.

검증된 GRID 인증 수집기를 그대로 재사용하고 영광 격자·기간·출력 경로만
독립 설정한다. 하루의 모든 필수 값이 완성된 경우에만 기존 수집기의
체크포인트/원자 저장 정책으로 보존한다. 인증키는 출력하지 않는다.
"""
...
OUTPUT_CSV = OUTPUT_DIR / "영광_격자예보_REH_POP_SKY_710일.csv"

# 영광 김삿갓1(plant_id=7912), 발전소 좌표
# 35.323668890815696, 126.486825627353의 KMA 동네예보 격자.
YEONGGWANG_NX = 52
YEONGGWANG_NY = 78
GRID_W = 149
...
def main() -> int:
    ...
    collector.NX = YEONGGWANG_NX
    collector.NY = YEONGGWANG_NY
    collector.IDX = (YEONGGWANG_NY - 1) * GRID_W + (YEONGGWANG_NX - 1)
