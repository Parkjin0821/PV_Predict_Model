"""Collect VEC(풍향) forecasts for all 710 Gwangju issue dates.

2026-08-19: confirmed via a live 1-hour test call that `vars=VEC` returns a
valid (non -99) value from the same `nph-dfs_shrt_grd` grid API already used
for TMP/SKY/REH/WSD/POP. The earlier AGENTS.md note claiming wind direction
was unavailable from this endpoint was wrong and untested; this corrects it.
Pressure (PRS/PA/PS) was also re-tested the same day and confirmed genuinely
absent from this API (all -99).

This is a versioned wrapper around the existing authenticated UCUBE collector,
same pattern as collect_gwangju_wsd_pop_710d_v1.py. It does not copy, display,
or log the KMA API key. Progress is saved daily and the same command can be
rerun to resume safely.
"""
...
OUTPUT_DIR = Path(
    ...
    r"\grid_forecast_vec_710d_v1_2026-08-19"
)
OUTPUT_CSV = OUTPUT_DIR / "광주_격자예보_VEC_710일.csv"
...
def validate_output(module) -> None:
    ...
    if (
        ...
        print("[PASS] VEC(풍향) 710일 수집 검증 완료")
    else:
...
def main() -> int:
    ...
    # Standalone source table. Do not merge into other grid-forecast files
    # until this one passes its own validation.
