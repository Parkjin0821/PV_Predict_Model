# -*- coding: utf-8 -*-
"""부안 요인재검증 2단계(다중공선성+모델선정) 러너 - 자정 예약용.

오늘(09-03) 1단계 산출물(요인재검증_v6_hourly_2026-09-03/요약.json)의
채택 후보를 그대로 불러와, 공통모듈
(factor_phase2_multicollinearity_modelselect_v1_2026-09-03.py)에
다중공선성 정리·LightGBM/XGBoost/선형회귀 비교를 맡긴다. 1단계를
다시 계산하지 않는다(재구현 아님, 이미 확정된 결과 재사용).
"""
...
def main() -> None:
    ...
    print(f"[부안] 1단계 채택값 로드: baseline {len(baseline_cols)}개, 후보 {len(candidate_cols)}개")

    phase1_mod = _load_module("buan_phase1_20260903", PHASE1_SCRIPT)
    phase2_mod = _load_module("phase2_common_20260903", PHASE2_MODULE)

    frame, _ = phase1_mod.build_hourly_frame(phase1_mod._load_module(
        "buan_v5_for_phase2", phase1_mod.V5_SCRIPT))
    days = pd.DatetimeIndex(np.sort(frame["issue_day"].unique()))
    folds = phase1_mod.expanding_folds_by_day(days, INITIAL_TRAIN_DAYS, TEST_BLOCK_DAYS) \
        if hasattr(phase1_mod, "expanding_folds_by_day") else None
    if folds is None:
        v5 = phase1_mod._load_module("buan_v5_for_folds", phase1_mod.V5_SCRIPT)
    ...
    phase2_mod.run_phase2(
        ...
        seed=SEED, out_dir=OUT_DIR, region_label="부안",
    )
