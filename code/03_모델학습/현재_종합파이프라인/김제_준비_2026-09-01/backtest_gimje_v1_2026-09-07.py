# -*- coding: utf-8 -*-
"""김제 D+1 총출력 공식모델(phase2+인근일사량, 09-07) 정식 백테스트.

영광·부안 백테스트와 동일 원리. 김제는 오늘(09-07) 전주146·정읍245
인근일사량을 추가한 버전이 새 공식모델이 됐으므로, **그 최신 공식본을
검증 대상으로 삼는다**(09-03 구버전이 아니라 09-07 채택본).
capacity_kw=1100(인버터 등록용량 합계 110kW×10대).
"""
...
OFFICIAL_PHASE2 = HERE / "outputs" / "요인재검증_v6_phase2_인근일사량추가_2026-09-07" / "phase2_요약.json"
OUT_DIR = HERE / "outputs" / "백테스트_v1_2026-09-07"
SEED = 42
TARGET = "plant_ac_power_kw"
INITIAL_TRAIN_DAYS = 90
TEST_BLOCK_DAYS = 30
CAPACITY_KW = 1100.0  # 인버터 등록용량 합계(110kW*10대)
...
def main() -> None:
    ...
    print(f"[김제] 공식채택 모델 재검증(인근일사량 포함): {model_name}, 특성 {len(features)}개")

    phase1_mod = _load_module("gimje_phase1_bt", PHASE1_SCRIPT)
    phase2_mod = _load_module("phase2_common_gimje_bt", PHASE2_MODULE)
    harness = phase2_mod._load_harness()
    bt_harness = _load_module("gwangju_harness_for_metrics_gimje", HARNESS_SCRIPT)
    ...
    v1 = _load_module("gimje_v1_for_folds_bt", phase1_mod.V1_SCRIPT)
    ...
    print(f"[김제] pooled 전체: MAE {pooled['MAE_kW']}kW, nMAE {pooled['nMAE_pct']}%, "
    ...
    print("[김제] 계절별:", json.dumps(seasonal, ensure_ascii=False))
    ...
    ax.set_ylabel("정규화 MAE, nMAE (%, 인버터등록용량 1,100kW 기준)")
    ax.set_title(f"김제 D+1 총출력({model_name}, 인근일사량 포함): 폴드별·계절별 nMAE")
    ...
    ax.set_ylabel("빈도")
    ax.set_title("김제 D+1 총출력: 잔차 분포(계통적 편향 확인)")
    ax.legend()
    fig.tight_layout()
    ...
    result = {
        "지역": "김제", "모델": model_name, "특성수": len(features),
