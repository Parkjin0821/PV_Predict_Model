# -*- coding: utf-8 -*-
"""일간 계층조정(hierarchical reconciliation) — v3(고정-tm) 재실행.

v1(08-20 밤)과 동일한 방법(직접 일간모델 + 낮시간 다중시각 시간모델합계를
OOF non-negative 가중결합)이지만, 두 하위모델 모두 v2(누출 데이터·
하드코딩 특성) 대신 **v2 스크립트**(`train_daily_official_v2_2026-08-21`,
`train_hourly_daytime_multihorizon_v2_2026-08-21` — v3 고정-tm 데이터+
폴드 내 특성 재선택)를 참조하도록 교체했다.
"""
...
OUT_DIR = ROOT / "outputs" / "일간_계층조정_v2_2026-08-21"
...
def hourly_sum_oof_predictions(df, windows, capacity_kw, seed):
    ...
    v2와 동일하게 목표시각·폴드마다 폴드 학습구간 안에서 특성을 재선택한다."""
...
def main() -> None:
    ...
    print("[2/3] 시간모델(06~19시 합계) OOF 예측(폴드별 특성 재선택)...")
