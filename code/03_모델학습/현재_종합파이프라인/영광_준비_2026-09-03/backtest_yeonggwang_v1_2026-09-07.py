# -*- coding: utf-8 -*-
"""영광 D+1 총출력 공식모델(phase2, 09-03) 정식 백테스트(09-07).

## 배경
사용자 질문("영광 백테스트 왜 없냐 → 지금 해야되는거잖아")으로 확인한 결과,
영광은 부안·김제와 달리 v6 파이프라인(phase1 특성선정→phase2 다중공선성+
모델선정) 이전의 "옛날 방식" 반복실험 단계 자체를 거친 적이 없다(데이터가
09-03 12:39에야 100% 준비돼서, 그때 이미 다듬어진 v6로 바로 들어감 -
09-03 12:39 절 참고). v6 파이프라인 자체엔 원래 백테스트 단계가 없어서
phase2의 kW 단위 MAE/RMSE 말고는 정식 검증 리포트가 지금까지 하나도 없었다.

## 이 스크립트가 하는 일(재구현 아니라 재사용)
- phase2 공식 채택 특성·모델(LightGBM)을 그대로 다시 로드(재선정 안 함 -
  이미 확정된 모델을 검증하는 것뿐).
- `factor_build_v6_hourly_yeonggwang_2026-09-03.py`의 frame·fold 생성
  로직 그대로 재사용.
- `backtest_harness_v1_2026-08-20밤.py`의 `metrics()` 함수를 그대로
  가져와 광주와 동일한 지표 체계(nMAE%·nRMSE%·WAPE%·SkillScore)를 쓴다 -
  새 지표를 발명하지 않음.
- capacity_kw=634(인버터 등록용량 합계, `Blockdata_등록용량_확정_
  2026-08-25` 감사에서 확인된 값 - 광주가 219kW 인버터합을 쓰는 것과
  동일 관례, 발전소 capacity 639.94kW가 아니라 인버터합을 쓴다).

## 추가한 것(v6 phase2에 없던 것)
각 시험(fold) 구간이 어느 계절에 속하는지 라벨을 붙여 **계절별 성능
breakdown**을 만든다 - 광주 백테스트 harness의 계절별 폴드(`_계절`)
관례를 영광에도 그대로 적용.

## 산출물
outputs/백테스트_v1_2026-09-07/
  백테스트_결과.json - 폴드별 전체 지표 + 계절별 요약
  01_계절별_nMAE_추이.png
  02_실측vs예측_산점도.png
  03_잔차_히스토그램.png
"""
...
PHASE1_SCRIPT = HERE / "factor_build_v6_hourly_yeonggwang_2026-09-03.py"
...
OFFICIAL_PHASE2 = HERE / "outputs" / "요인구축_v6_phase2_2026-09-03" / "phase2_요약.json"
OUT_DIR = HERE / "outputs" / "백테스트_v1_2026-09-07"
SEED = 42
TARGET = "plant_ac_power_kw"
INITIAL_TRAIN_DAYS = 90
TEST_BLOCK_DAYS = 30
CAPACITY_KW = 634.0  # 인버터 등록용량 합계(50kW*12+34kW*1) - 광주 219kW 관례와 동일하게 인버터합 사용
...
def main() -> None:
    ...
    print(f"[영광] 공식채택 모델 재검증: {model_name}, 특성 {len(features)}개")

    phase1_mod = _load_module("yg_phase1_bt", PHASE1_SCRIPT)
    phase2_mod = _load_module("phase2_common_bt", PHASE2_MODULE)
    harness = phase2_mod._load_harness()  # make_model 재사용
    bt_harness = _load_module("gwangju_harness_for_metrics", HARNESS_SCRIPT)  # metrics() 재사용

    frame, _ = phase1_mod.build_hourly_frame()
    days = pd.DatetimeIndex(np.sort(frame["issue_day"].unique()))
    folds = phase1_mod.expanding_folds_full_coverage(days, INITIAL_TRAIN_DAYS, TEST_BLOCK_DAYS)
    ...
    for i, (train_days, test_days) in enumerate(folds, start=1):
        ...
        season_label = season_counts.idxmax()  # 이 30일 블록의 대표 계절(월 최빈값)

        m = bt_harness.metrics(y_true, pred, CAPACITY_KW,
    ...
    print(f"[영광] pooled 전체: MAE {pooled['MAE_kW']}kW, nMAE {pooled['nMAE_pct']}%, "
    ...
    for season in ["봄", "여름", "가을", "겨울"]:
        ...
        if mask.sum() < 10:
            ...
            continue
        sm = bt_harness.metrics(true_all[mask], pred_all[mask], CAPACITY_KW)
        seasonal.append({"계절": season, **sm})
    print("[영광] 계절별:", json.dumps(seasonal, ensure_ascii=False))

    # === 차트 1: 폴드별 nMAE 추이(계절 라벨) ===
    fdf = pd.DataFrame(fold_rows)
    fig, ax = plt.subplots(figsize=(11, 5.5))
    season_color = {"봄": "#57a06b", "여름": "#e0576f", "가을": "#e07a3f", "겨울": "#5b8def"}
    for season, group in fdf.groupby("계절"):
        ax.scatter(group["폴드"], group["nMAE_pct"], label=season,
                   color=season_color.get(season, "#888"), s=60, zorder=3)
    ax.plot(fdf["폴드"], fdf["nMAE_pct"], color="#ccc", linewidth=1, zorder=1)
    ax.set_xlabel("폴드 번호(walk-forward, 시험블록 30일씩)")
    ax.set_ylabel("정규화 MAE, nMAE (%, 인버터등록용량 634kW 기준)")
    ax.set_title(f"영광 D+1 총출력({model_name}): 폴드별·계절별 nMAE")
    ...
    # === 차트 2: 실측 vs 예측 산점도(계절별 색상) ===
    fig, ax = plt.subplots(figsize=(7, 7))
    for season in ["봄", "여름", "가을", "겨울"]:
        mask = season_all == season
        if mask.sum() == 0:
            continue
        ax.scatter(true_all[mask], pred_all[mask], s=5, alpha=0.3,
                  color=season_color.get(season), label=season)
    ...
    ax.set_ylabel("빈도")
    ax.set_title("영광 D+1 총출력: 잔차 분포(계통적 편향 확인)")
    ax.legend()
    fig.tight_layout()
    ...
    result = {
        "지역": "영광", "모델": model_name, "특성수": len(features),
