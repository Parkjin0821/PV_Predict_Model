# -*- coding: utf-8 -*-
"""영광 요인구축 2단계(다중공선성+모델선정) 러너 - 자정 예약용.
부안·김제 러너와 동일 원리, 경로만 교체. 영광은 1단계 산출물 키가
"v6_결과"(지속성 대비)라 지속성 비교 컬럼만 참고용으로 다르고, 나머지
(선택_baseline/선택_후보 로드→phase2 공통모듈 위임)는 완전히 동일.
"""
...
PHASE1_SUMMARY = HERE / "outputs" / "요인구축_v6_hourly_2026-09-03" / "요약.json"
PHASE2_MODULE = PIPELINE_ROOT / "factor_phase2_multicollinearity_modelselect_v1_2026-09-03.py"
OUT_DIR = HERE / "outputs" / "요인구축_v6_phase2_2026-09-03"
SEED = 42
...
def main() -> None:
    ...
    print(f"[영광] 1단계 채택값 로드: baseline {len(baseline_cols)}개, 후보 {len(candidate_cols)}개")

    phase1_mod = _load_module("yg_phase1_20260903", PHASE1_SCRIPT)
    phase2_mod = _load_module("phase2_common_yg_20260903", PHASE2_MODULE)
    ...
    phase2_mod.run_phase2(
        ...
        seed=SEED, out_dir=OUT_DIR, region_label="영광",
    )
