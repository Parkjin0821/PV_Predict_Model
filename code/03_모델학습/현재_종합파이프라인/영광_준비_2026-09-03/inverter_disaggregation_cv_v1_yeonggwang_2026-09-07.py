# -*- coding: utf-8 -*-
"""영광 인버터 분해 CV(방법론①, 09-07) - 광주 08-27/김제 09-03 "사전등록기준"
방법론을 영광에 적용(오늘 만든 방법론②와의 직접비교용).

## 배경(★사용자 확정: "1,2를 비교해서 전체적으로 더 잘 나오는 걸로
선정"★)
09-07 오늘 만든 `inverter_disaggregation_ABC_v1_yeonggwang_2026-09-07.py`
(방법론② - 부안 08-31 개정판 계승, 실측 일간총량을 인버터별로 분해,
사전문턱 없이 최저 nMAE 채택)와, 광주 08-27/김제 09-03이 쓰는
방법론①(총출력 **예측모델**의 예측값을 인버터별로 분해, 실행 전
고정한 통과기준 - MAE·RMSE 1%↑개선 등 - 을 충족해야만 A 대신 채택)을
영광에도 똑같이 돌려서 **같은 지역·같은 데이터로 두 방법론을 직접
비교**한다.

## 재사용한 것(재구현 안 함)
- 총출력 모델: `backtest_yeonggwang_v1_2026-09-07.py`가 쓰는 phase2
  공식채택모델(LightGBM, 특성·하이퍼파라미터 전부 그대로) 그대로 재사용 -
  `factor_build_v6_hourly_yeonggwang_2026-09-03.py`의 build_hourly_frame·
  expanding_folds_full_coverage, `factor_phase2_multicollinearity_
  modelselect_v1_2026-09-03.py`의 harness.make_model.
- 인버터 분해 CV 골격(_score/_adjudicate/_predict_a/b/c, 사전등록규칙
  문구·임계값): 김제 `inverter_disaggregation_cv_v1_gimje_2026-09-03.py`
  그대로(로직 100% 동일, 인버터수·용량표만 영광 것으로 교체).

## 영광 고유 사항(광주·김제와 다른 점)
- 인버터 13기, **12기 50kW+1기 34kW로 김제(10기 전부 110kW 균등)와
  달리 균등하지 않음** - A는 광주(5기 이종용량)처럼 "정격용량비례"로
  일반화(균등1/13 아님).
- 총출력모델이 김제처럼 티어 구분 없는 단일 D+1 hourly 트랙(영광
  v6 phase2 공식모델).
"""
...
OUT = HERE / "outputs" / "인버터_분해교차검증_v1_yeonggwang_2026-09-07"
...
N_INVERTERS = 13
CAPACITY = np.array([50.0] * 12 + [34.0], dtype=float)  # 등록용량 확인값(09-07 실측)
CAPACITY_SHARE = CAPACITY / CAPACITY.sum()
SEED = 42
TOL_KW = 1.0

PREREGISTERED_RULES = {
    "판정단위": "영광_총출력(단일 D+1 hourly 트랙, phase2 공식모델)",
    "기준선": "A_정격용량비례",
    "규칙1": "A 대비 전체 MAE와 RMSE가 모두 1% 이상 개선",
    "규칙2": "어느 폴드에서도 RMSE가 A보다 5% 이상 악화되지 않음",
    "규칙3": "각 인버터 nMAE 악화가 A 대비 1.0%p 이내",
    "규칙4": "동일 시험행 100% 사용 및 13대 예측합계 오차 1.0kW 이하",
    "동률": "RMSE가 사실상 같으면 A→B→C 순으로 단순한 방법 우선",
    "주의": "결과를 보기 전에 코드에 고정한 규칙이며 실행 후 변경하지 않음",
    "광주_08-27_김제_09-03_대비_차이": "인버터수 13(12x50kW+1x34kW, 균등 아님 - "
                                    "그래서 A는 광주식 정격용량비례), 총출력모델은 "
                                    "영광 v6 phase2 공식모델(단일 D+1 트랙, 김제와 동일 구조).",
}
...
def _adjudicate(detail: pd.DataFrame, folds: pd.DataFrame, predictions: pd.DataFrame) -> pd.DataFrame:
    method_order = {"A_정격용량비례": 0, "B_계절시간중앙값": 1, "C_LightGBM비중": 2}
    rows = []
...
def main() -> None:
    ...
    print(f"[영광] 총출력 공식모델 재사용: {model_name}, 특성 {len(features)}개")

    phase1_mod = _load_module("yg_phase1_invcv", PHASE1_SCRIPT)
    phase2_mod = _load_module("phase2_common_invcv", PHASE2_MODULE)
    harness = phase2_mod._load_harness()

    frame, _ = phase1_mod.build_hourly_frame()
    TARGET = phase1_mod.TARGET if hasattr(phase1_mod, "TARGET") else "plant_ac_power_kw"
    ...
    for fold_i, (train_days, test_days) in enumerate(folds, start=1):
        ...
        if full_ok.sum() < 30:
            print(f"[폴드{fold_i}] 13기 전부가용 시험행 부족({full_ok.sum()}) - 생략")
            continue
        ...
        share_predictions = {
            "A_정격용량비례": _predict_a(len(test_full)),
    ...
    if max_diff > TOL_KW:
        raise AssertionError(f"13대 예측합계 reconciliation 실패: {max_diff}")
    ...
    print("\n=== 영광 인버터 분해 사전등록 판정(방법론①) ===")
    print(decision.to_string(index=False))
    print(f"\n13대 합계 최대차이: {max_diff:.3e} kW")
