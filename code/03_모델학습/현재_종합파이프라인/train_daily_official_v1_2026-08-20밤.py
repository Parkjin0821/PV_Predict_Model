# -*- coding: utf-8 -*-
"""일간 총량 직접모델 — 공식 KMA 파이프라인으로 전면 재작업.

## 왜 다시 만드는가 (사용자 지시, 08-20 밤)
기존 `train_daily_and_reconcile.py`는 `open_meteo_forecast_*`(비공식)에
의존하고, `HOUR_ENSEMBLE`(1시간_앙상블 — `build_hourly_ensemble.py`
산출물)도 마찬가지로 비공식 자료 기반 구버전 1시간 예측을 참조한다.
"현재 만드는 모델에 남은 Open-Meteo 흔적을 전부 걷어내라"는 지시에 따라,
일간 모델을 공식 KMA 자료만으로 다시 만든다.

## 이번 버전의 범위(명시적 축소, 이유 포함)
원래 스크립트는 ①직접 일간모델 + ②당일 10시 발행 1시간모델의 낮시간
합계 + ③ 그 둘의 계층조정(hierarchical reconciliation)까지 했다.
**이번엔 ①직접 일간모델까지만 한다.** ②③은 "내일 하루 낮시간 전체
(10~14개 시각)"에 대한 다중시각 1시간 예측이 있어야 하는데, 지금까지
검증한 건 단일 수평(+1h/+24h/+48h) 스팟체크뿐이고 하루 전체를 훑는
파이프라인은 아직 없다 — 별도 과제로 명시 이월(AGENTS.md에 기록).

## 특성 구성
- 지속성 특성(발전량 자기이력): 기존과 동일, Open-Meteo 무관.
- **미래 예보 특성**: `open_meteo_forecast_*` 대신 공식 병합 데이터셋
  (`gwangju_1hour_model_dataset_official_v2_2026-08-20밤.csv`)의 NWP
  계열(DSWRF·TCDC·LCDC·MCDC·HCDC·DSWRFLX/DIFSWRF_bsrn정제·REH·POP·SKY·
  TMP·WSD·VEC)을 **목표일(D+1) 날짜로 집계**(mean/min/max/sum)해서 쓴다.
  이 값들은 이미 청천모양 보존적 재분배로 1시간 해상도이며, 발표일=D→
  D+1 관례상 항상 사전 확보된 예보이므로 일간모델 발행시점 기준으로도
  누출이 아니다.
- **장비 특성**: `mean_inverter_temperature_c`는 Blockdata 실시간 API에
  없다는 08-20 밤 확인에 따라 일간모델에서도 제외한다(시간단위 최종
  14개 결정과 일관성 유지).
"""
...
OUT = ROOT / "outputs" / "일간_직접모델_v1_2026-08-20밤"
HOURLY_CSV = Path(
