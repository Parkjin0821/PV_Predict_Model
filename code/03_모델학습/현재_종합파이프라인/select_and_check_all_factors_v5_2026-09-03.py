# -*- coding: utf-8 -*-
"""5번+5-2번 재실행(★5차, 09-03★) - v4에 이어 사용자가 09-03 아침에 재확정한
4대 요인 정의(메일 기준 설비요인 범위 포함)와 전체 대조해 빠진 것 보완.

## v4 대비 변경(09-03 대조 결과)
사용자가 준 정확한 항목 리스트(환경 9종·시간 4종·설비는 메일 기준
수집가능분만·지형은 가능하면)와 v4의 48개 후보를 전부 대조한 결과:
- 환경요인 9종(강우량·적설량·적운량·풍속·기압·일조량·일사량·습도·기온)
  **전부 이미 포함돼 있었음** - 추가 없음.
- 시간요인 4종(월·일·시간·연) **전부 이미 포함**(v4에서 이미 반영) -
  추가 없음.
- 설비요인(08-26 Blockdata 메일 기준 실제 수집 가능한 것만): 모듈표면
  온도(추정치)·출력온도(추정치)는 이미 있음, 방위각·설치각도·열화현상은
  메일에 없는 필드라 원천적으로 미확보 - 제외 유지가 맞음(임의 판단
  아니라 API 응답에 아예 없는 필드). **단 "인버터 효율"은 계산 가능한데
  후보에 빠져있었다** - `plant_output_kw/plant_input_power_kw` 비율로
  신규 추가(누출 방지 위해 둘 다 발전출력_1시간전_kW와 동일한 shift(1)
  정렬 재사용).
- 지형요인(지역·해발·초상온도·지면온도): 지면온도는 이미 있음. 지역·
  해발은 광주 단일 발전소 데이터셋에서 상수(분산 0)라 특성으로서
  무의미해 구조적으로 제외. 초상온도는 별도 파일 병합 미완료 상태 -
  "안 되면 제외" 지시에 따라 이번 라운드는 제외(추후 병합되면 재검토).
"""
...
OUT_DIR = Path(
    r"C:\Users\u-cube\JIN\코덱스\결과물\예측모델\광주\전체요인_재검증_v5_2026-09-03")
SEED = 42
THRESHOLDS = [0.0, 0.05, 0.1, 0.15, 0.2, 0.3, 0.4]
VIF_SEVERE, PAIR_CORR_HIGH = 10.0, 0.8

# 09-03 신규: 08-26 Blockdata 메일 기준 실제 수집 가능한 설비요인 중
# "인버터 효율"이 계산 가능한데도 v4 후보에 빠져있었다 - 발전출력(AC)/
# 입력전력(DC) 비율. 둘 다 STARTLABEL(시작시각 라벨)이라 다른 장비
# 텔레메트리와 동일하게 shift(1)로 정렬해 누출을 막는다(재구현 아님 -
# STARTLABEL_COLUMNS와 동일한 정렬 규약 재사용).
EQUIP_EXTRA_COLUMNS = ["인버터효율_pct"]


def build_equip_extra_frame(df: pd.DataFrame) -> pd.DataFrame:
    ac = df["plant_output_kw"].shift(1)
    dc = df["plant_input_power_kw"].shift(1)
    out = pd.DataFrame(index=df.index)
    out["인버터효율_pct"] = (ac / dc.replace(0, np.nan) * 100).clip(0, 100)
    return out
...
def main() -> None:
    ...
    equip_extra_cand = build_equip_extra_frame(df)   # 09-03 신규: 인버터효율
    cand = pd.concat([env_equip_cand, time_cand, equip_extra_cand], axis=1)
    all_cols = sel.CANDIDATE_COLUMNS + TIME_FACTOR_COLUMNS + EQUIP_EXTRA_COLUMNS
