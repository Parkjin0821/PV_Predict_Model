# -*- coding: utf-8 -*-
"""5-2번 재실행(08-20 밤) — v2(정렬수정+1시간해상도) 18개 특성의 다중공선성 점검."""
...
spec = importlib.util.spec_from_file_location(
    "sel_v2", Path(__file__).parent / "select_features_by_correlation_threshold_v2_2026-08-20밤.py"
)
sel = importlib.util.module_from_spec(spec)
spec.loader.exec_module(sel)

CORR_DIR = Path(r"C:\Users\u-cube\JIN\코덱스\결과물\예측모델\광주\correlation_threshold_selection_v2_2026-08-20밤")
OUT_DIR = Path(r"C:\Users\u-cube\JIN\코덱스\결과물\예측모델\광주\multicollinearity_check_v2_2026-08-20밤")
VIF_SEVERE, PAIR_CORR_HIGH = 10.0, 0.8

SELECTED_18 = [
    "plant_input_power_kw", "plant_input_current_a", "DSWRF", "기상청관측_일사량_W_m2",
    "mean_power_factor", "기상청관측_일조시간_hr", "DSWRFLX_bsrn정제", "mean_input_voltage_v",
    "REH", "mean_inverter_temperature_c", "기상청관측_전운량_pct", "추정_출력온도",
    "POP", "TCDC", "LCDC", "추정_모듈표면온도", "기상청관측_상대습도_pct", "SKY",
]
...
def main() -> None:
    ...
    daylight_cand = cand[daylight_next > 0]
    ...
    def evaluate(name, cols):
        ...
        for col in cols:
            prefix = "목표_" if col in sel.FORECAST_COLUMNS else "발행시_"
            base[f"{prefix}{col}"] = cand[col]
        base["목표_발전출력_kW"] = target_next
        base["목표_낮시간"] = daylight_next
